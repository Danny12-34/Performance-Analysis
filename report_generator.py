...
def generate_pdf(df, class_name, subject_name, teacher_name, filename="report.pdf"):
    ...
    table_data = [columns] + [[row[col] for col in columns] for _, row in sorted_df.iterrows()]
    col_widths = [160 if "Name" in c else 100 if "Category" in c else 70 for c in columns]
    ...
    for i in range(1, len(table_data)):
        ...
        color = colors.lightgreen if cat=="Excellent" else colors.lightblue if cat=="Good" else colors.lightyellow if cat=="Fair" else colors.salmon
    ...
    if len(weak_students)==0:
        ...
        weak_data = [["Student Name","Percentage","Status"]]+[[row["Student Name"], f"{row['Percentage']}%", "Needs Support"] for _, row in weak_students.iterrows()]
